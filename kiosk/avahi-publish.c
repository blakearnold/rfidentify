...
static void create_services(AvahiClient *c) {
    ...
    if (avahi_entry_group_is_empty(group)) {
        ...
        if ((ret = avahi_entry_group_add_service(group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, "_rfid-srv._tcp", NULL, NULL, 515, NULL)) < 0) {

            if (ret == AVAHI_ERR_COLLISION)
                goto collision;

            fprintf(stderr, "Failed to add _rfid._server._tcp service: %s\n", avahi_strerror(ret));
            goto fail;
        }

        /* Add an additional (hypothetic) subtype */
        if ((ret = avahi_entry_group_add_service_subtype(group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, name, "_rfid-srv._tcp", NULL, "_magic._sub._rfid-srv._tcp") < 0)) {
            fprintf(stderr, "Failed to add subtype _magic._sub._rfid._server._tcp: %s\n", avahi_strerror(ret));
            goto fail;
        }
...
static void modify_callback(AVAHI_GCC_UNUSED AvahiTimeout *e, void *userdata) {
    ...
    avahi_free(name);
    name = avahi_strdup("Modified RFID SERVER");
